...
import StudentSidebarContent from './StudentSidebaritems';
...
const StudentInnerSidebar = () => {
  ...
  useEffect(() => {
    const result = findActiveUrl(StudentSidebarContent, pathname);
    if (result) {
      ...
      <SimpleBar className="md:h-[calc(100vh_-_225px)] h-[calc(100vh_-_200px)] pt-4 ">
        <SidebarItems className=" mt-2 ">
          <SidebarItemGroup className="sidebar-nav">
            {StudentSidebarContent.map((item, index) => (
...
export default StudentInnerSidebar;
