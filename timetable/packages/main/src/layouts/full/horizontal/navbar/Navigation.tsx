          ...
          {Menuitems.map((item) => {
            ...
            return (
              ...
              {item.children ? (
                ...
                >
                  <p
                    className={`w-full ${isActive
                      ? ` text-${item.bg} bg-light${item.bg} dark:bg-light${item.bg} dark:text-${item.bg} `
                      : `  group-hover:text-${item.bg} before:content-['']  before:absolute before:start-0 before:top-0 before:h-full before:w-0 hover:before:w-full before:bg-light${item.bg}  before:transition-all before:duration-400 before:rounded-full`
                      } py-2.5 px-5 rounded-full flex gap-3 items-center text-ld`}
                  >
                    <Link to={item.href}>
                ...
                <Link to={item.href}>
                  <p className={`py-2 px-3 rounded-full flex gap-3 items-center ${active === item.id ? 'bg-error text-white' : 'group-hover/nav:bg-primary group-hover/nav:text-primary'}`}>
