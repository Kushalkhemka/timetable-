  ...
  return (
    <div className="overflow-x-auto">
      <div className="flex justify-between overflow-x-auto mb-8 gap-6 ">
        {/* Summary sections */}
        <div
          className={`flex gap-3 items-center sm:w-3/12 w-full mb-2 cursor-pointer p-5 rounded-lg hover:bg-lightinfo dark:hover:bg-lightinfo ${activeTab == "All" ? "bg-lightinfo dark:bg-lightinfo/30" : null
            }   `}
        ...
        <div
          className={`flex gap-3 items-center sm:w-3/12 w-full mb-2 cursor-pointer p-5 rounded-lg hover:bg-lightinfo dark:hover:bg-lightinfo ${activeTab == "Shipped" ? "bg-lightinfo dark:bg-lightinfo/30" : null
            }`}
        ...
        <div
          className={`flex gap-3 items-center sm:w-3/12 w-full mb-2 cursor-pointer p-5 rounded-lg hover:bg-lightinfo dark:hover:bg-lightinfo ${activeTab == "Delivered" ? "bg-lightinfo dark:bg-lightinfo/30" : null
            }`}
        ...
        <div
          className={`flex gap-3 items-center sm:w-3/12 w-full mb-2  cursor-pointer p-5 rounded-lg hover:bg-lightinfo dark:hover:bg-lightinfo ${activeTab == "Pending" ? "bg-lightinfo dark:bg-lightinfo/30" : null
            }`}
                  ...
                  <TableCell className="text-center">
                    <div className="flex justify-center gap-3">
                      <Tooltip content="Edit Invoice" placement="bottom">
                        <Button className="btn-circle p-0 mb-2 bg-lightsuccess text-success dark:text-success hover:bg-success dark:bg-lightsuccess dark:hover:bg-success group/link">
                          <Link to={`/apps/invoice/edit/${decodeURIComponent(invoice.billFrom)}`}>
                            <Icon icon="solar:pen-outline" className=" group-hover/link:text-white" height={18} />
                          </Link>
                        </Button>
                      </Tooltip>
                      <Tooltip content="View Invoice" placement="bottom">
                        <Button color={"lightprimary"} className="btn-circle p-0 mb-2 dark:text-primary dark:hover:text-white group/link">
                          <Link to={`/apps/invoice/detail/${decodeURIComponent(invoice.billFrom)}`}>
                            <Icon icon="solar:eye-outline" height={18}  className="group-hover/link:text-white"/>
                          </Link>
                        </Button>
                      </Tooltip>
                      <Tooltip content="Delete Invoice" placement="bottom">
                        <Button
                          color={"lighterror"}
                          className="btn-circle p-0 mb-2 dark:text-error dark:hover:text-white group/link" 
                          onClick={() => {
                            setSelectedProducts([invoice.id]);
                            handleDelete();
                          }}
                        >
                          <Icon icon="solar:trash-bin-minimalistic-outline" className="group-hover/link:text-white" height={18} />
                        </Button>
                      </Tooltip>
                    </div>
                  </TableCell>
