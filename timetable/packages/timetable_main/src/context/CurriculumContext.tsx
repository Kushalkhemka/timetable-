...
interface CurriculumContextType {
  ...
  approveCurriculumWithCourseCode: (curriculum: CurriculumData) => Promise<boolean>;
}
...
export const CurriculumProvider: React.FC<CurriculumProviderProps> = ({ children }) => {
  ...
  const loadUploadedFilesFromDatabase = async () => {
    try {
      ...
      if (dbCurricula.length > 0) {
        // Only show approved/active to avoid re-showing deleted drafts
        const visible = dbCurricula.filter(c => c.status === 'approved' || c.status === 'active');
        const uploadedFiles: UploadedFile[] = visible.map((curriculum, index) => ({
          id: `db-${curriculum.id}`,
  ...
  const processOCR = async (fileId: string): Promise<void> => {
    ...
    try {
      ...
      const response = await fetch(`${ingestBase || ''}/ingest`, {
        method: 'POST',
        body: form
      });
  ...
  const processOCRDirect = async (file: UploadedFile): Promise<void> => {
    ...
    try {
      ...
      const ingestBase2 = ((import.meta as any).env?.VITE_INGEST_URL || '').replace(/\/$/, '');
      const response = await fetch(`${ingestBase2 || ''}/ingest`, {
        method: 'POST',
        body: form
      });
  ...
  // Approve curriculum by prompting for course code, validate in DB, and update status
  const approveCurriculumWithCourseCode = async (curriculum: CurriculumData): Promise<boolean> => {
    try {
      const code = window.prompt('Enter course code to link this curriculum (e.g., CS201):', '');
      if (!code) return false;
      const trimmed = code.trim();
      if (!trimmed) return false;

      let subject = await curriculumService.getSubjectByCode(trimmed);
      if (!subject) {
        const wantsCreate = window.confirm('Course code not found. Create subject now?');
        if (!wantsCreate) return false;
        const created = await curriculumService.upsertSubject({
          code: trimmed,
      ...
      // If a curriculum for this subject+dept already exists, just mark it approved; otherwise insert once
      const exists = await curriculumService.curriculumExists(curriculum.subjectName, curriculum.department);
      if (exists) {
        const ok = await curriculumService.updateCurriculumStatus({ subjectName: curriculum.subjectName, department: curriculum.department, status: 'approved' });
        if (!ok) {
          window.alert('Failed to update existing curriculum status.');
          return false;
        }
      } else {
        const savedOk = await saveCurriculumToDatabase({ ...curriculum, status: 'approved' });
        if (!savedOk) {
          window.alert('Failed to save curriculum.');
          return false;
        }
      }

      // Local state update: mark approved
      updateCurriculum(curriculum.id, { status: 'approved' });
      window.alert(`Curriculum approved and linked to ${subject.code} - ${subject.name}.`);
      return true;
    } catch (e) {
      console.error('Error approving curriculum:', e);
      return false;
    }
  };
  ...
  const value: CurriculumContextType = {
    ...
    approveCurriculumWithCourseCode
  };
