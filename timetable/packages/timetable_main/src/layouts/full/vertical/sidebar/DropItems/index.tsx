...
const Dropitms: React.FC<NavItemsProps> = ({ item }) => {
        ...
        <SidebarItem
          ...
          className={`realtive mb-1 sidebar-link  relative  py-0 px-6  ${
            item.url == pathname
              ...
              : ` text-link bg-transparent dark:bg-transparent group/link before:content-[''] before:absolute before:left-0 before:top-0 before:h-full before:w-0 hover:before:w-full before:bg-light${item.bg} before:transition-all before:duration-400 before:rounded-e-full hover:bg-transparent dark:hover:bg-transparent hover:text-${item.bg} dark:hover:text-${item.bg} `
          } `}
        >
          <span
          ...
          >
            ...
            <span className={` flex flex-auto hide-menu-flex`}>
              <span
                className={`max-w-36 drop-items w-full overflow-hidden  ${
                  item.url == pathname ? '!text-ld' : ''
                } `}
              >
                {t(`${item.name}`)}
              </span>
