...
// Enhanced AI Agent with Chain of Thought and Tree of Thought prompting
export interface TimetableEntity {
  id: string;
  type: 'class' | 'teacher' | 'room' | 'period' | 'slot';
  label: string;
  context?: any;
}
...
// Persistent memory record stored in Supabase
...
export class TimetableAIAgent {
  ...
  private shortTermMemory: Array<{ timestamp: number; userInput: string; entities: TimetableEntity[]; intent: IntentType }>= [];
  private readonly memoryLimit = 50;
  // Rate limiting for Gemini API calls
  private lastApiCall = 0;
  private readonly apiCallDelay = 0; // No rate limiting for this API key
  ...
  private intentKeywords: Record<IntentType, RegExp[]> = {
  ...
  private detectIntent(userInput: string): IntentType {
    const text = userInput || '';
    const intents = Object.keys(this.intentKeywords) as IntentType[];
    for (const intent of intents) {
      const regs: RegExp[] = this.intentKeywords[intent];
      if (regs.every((re) => re.test(text))) return intent as IntentType;
      if (regs.some((re) => re.test(text))) return intent as IntentType;
    }
    if (/\b(check|find|show)\b.*\b(slot|availability|free)\b/i.test(text)) return 'AVAILABILITY';
    return 'SCHEDULE';
  }
...
Return ONLY valid JSON: { "intent": "EXACT_INTENT_NAME" }`;
    try {
      ...
      if (allowed.includes(intent)) return intent as IntentType;
  ...
  private remember(userInput: string, entities: TimetableEntity[], intent: IntentType): void {
    this.shortTermMemory.push({ timestamp: Date.now(), userInput, entities, intent });
    if (this.shortTermMemory.length > this.memoryLimit) this.shortTermMemory.shift();
  }

  private async verifyAction(action: any): Promise<{ ok: boolean; reason?: string }> {
    ...
    try {
      if (action.type === 'SCHEDULE') {
        ...
        if (conflict) return { ok: false, reason: 'Scheduling conflict for class/teacher/room at the selected time' };
      }
      if (action.type === 'RESCHEDULE') {
        ...
        if (conflict) return { ok: false, reason: 'Reschedule would create a conflict' };
      }
      if (action.type === 'SUBSTITUTE') {
        const p = action.parameters || {};
        const busy = await this.isTeacherBusy(p.substituteTeacher, p.day, p.period);
        if (busy) return { ok: false, reason: 'Substitute teacher is busy at that time' };
      }
      return { ok: true };
    } catch (_e) {
  ...
  private async hasConflict(
    opts: { day: string; period: string; className?: string; teacher?: string; room?: string },
    excludeId?: string
  ): Promise<boolean> {
    ...
    const rows = (data || []).filter((r: any) => (excludeId ? r.id !== excludeId : true));
    return rows.some((r: any) =>
      (opts.className && r.students === opts.className) ||
      (opts.teacher && r.instructor_name === opts.teacher) ||
      (opts.room && r.room === opts.room)
    );
  }

  private async isTeacherBusy(teacher: string, day: string, period: string): Promise<boolean> {
  ...
  public async findFreeSlots(
    scope: { className?: string; teacher?: string; room?: string },
    day: string
  ): Promise<string[]> {
    ...
    const busy = new Set(
      (data || [])
        .filter((r: any) =>
          (scope.className && r.students === scope.className) ||
  ...
  private async findAvailableTeacherForSubject(
    subject: string,
    className: string,
    day: string,
    period: string
    ...
    try {
      ...
      for (const t of unique) {
        const busy = await this.isTeacherBusy(t as string, day, period);
        if (!busy) return t as string;
      }
      return undefined;
    } catch {
      return undefined;
  ...
  private async saveMemoryRecord(record: AgentMemoryRecord): Promise<void> {
    try {
  ...
  private async getRecentMemory(limit = 20): Promise<AgentMemoryRecord[]> {
    try {
      const { data, error } = await this.supabase
        .from('ai_agent_memory')
        .select('*')
      ...
      console.warn('Memory fetch skipped:', (error as any)?.message || error);
      return [];
    }
  }
  ...
  private formatContextForPrompt(context: any): string {
    ...
    const entitiesByType = context.entities.reduce((acc: any, entity: TimetableEntity) => {
    ...
    Object.entries(entitiesByType).forEach(([type, entities]: [string, any]) => {
      contextText += `\n=== ${type.toUpperCase()} ENTITIES ===\n`;
      
      entities.forEach((entity: TimetableEntity, index: number) => {
        ...
        if (entity.context) {
          ...
          if (entity.type === 'class') {
            ...
            if (ctx.scheduleByDay) {
              ...
              Object.entries(ctx.scheduleByDay).forEach(([day, classes]: [string, any]) => {
                if (classes && classes.length > 0) {
                  contextText += `- ${day}: ${classes.map((c: any) => `${c.period} - ${c.course} (${c.instructor})`).join(', ')}\n`;
                }
              });
            }
            ...
            if (ctx.schedule && ctx.schedule.length > 0) {
              ...
              ctx.schedule.forEach((cls: any, idx: number) => {
            ...
            if (ctx.scheduleByDay) {
              ...
              Object.entries(ctx.scheduleByDay).forEach(([day, classes]: [string, any]) => {
                if (classes && classes.length > 0) {
                  contextText += `- ${day}: ${classes.map((c: any) => `${c.period} - ${c.course} (${c.students})`).join(', ')}\n`;
                }
              });
            }
  ...
  private async generateCoTResponse(userInput: string, context: any): Promise<CoTStep[]> {
    const cotPrompt = `
...
`;
  ...
  private async generateTreeOfThought(userInput: string, context: any): Promise<ThoughtNode> {
    const totPrompt = `
...
`;
  ...
  private async parseContextualMentions(input: string): Promise<TimetableEntity[]> {
...
User Input: "${input}"`;
    try {
      ...
      const entities: TimetableEntity[] = [];
      for (const it of items) {
        ...
        if (['class','teacher','room','period','subject'].includes(type)) {
          ...
          if (type === 'class') context = await this.getClassContext(label);
          if (type === 'teacher') context = await this.getTeacherContext(label);
          if (type === 'room') context = await this.getRoomContext(label);
          entities.push({ id: `${type}:${label}`, type: (type as any), label, context });
        }
      }
      console.log('📊 LLM entities parsed:', entities.length);
      return entities;
    } catch (e) {
      console.warn('LLM entity extraction failed, returning empty entities:', (e as any)?.message || e);
      return [];
    }
  }

  // Identify mention type and get context
  private async identifyAndGetContext(mention: string): Promise<{ type: string; data: any } | null> {
    try {
  ...
  private async getClassContext(className: string): Promise<any> {
    try {
      ...
      const scheduleByDay = classes.reduce((acc: any, cls: any) => {
  ...
  private async getTeacherContext(teacherName: string): Promise<any> {
    try {
      ...
      const scheduleByPeriod = classes.reduce((acc: any, cls: any) => {
  ...
  private async getRoomContext(roomName: string): Promise<any> {
    try {
  ...
  public async processRequest(userInput: string): Promise<AIAgentResponse> {
    ...
    try {
      ...
      this.saveMemoryRecord({ user_input: userInput, intent, entities }).catch((e: any) => {
...
`;
    ...
    try {
      ...
      if (response) {
        ...
        console.warn('⚠️ Gemini returned null/empty response');
      }
    } catch (error) {
    ...
    // Enhanced fallback with basic processing
    console.log('🔄 Using fallback response due to API failure');
    
    // Try to provide a basic response based on intent detection
    const detectedIntent = this.detectIntent(userInput);
    const entities = await this.parseContextualMentions(userInput);
    
    let fallbackResponse = `I'm having trouble connecting to my reasoning system right now. This could be due to API rate limits or configuration issues. Please try again in a moment, or contact support if the issue persists.`;
    
    // Provide basic responses for common requests
    if (detectedIntent === 'SCHEDULE' && userInput.toLowerCase().includes('add')) {
      fallbackResponse = `I understand you want to add a class to the timetable. However, I'm currently unable to process this request due to a technical issue. Please try again in a few moments, or use the manual timetable creation tools in the meantime.`;
    } else if (detectedIntent === 'AVAILABILITY') {
      fallbackResponse = `I'd like to help you find available time slots, but I'm experiencing technical difficulties. Please try the "View Timetable" feature to manually check availability, or try again in a few moments.`;
    }
    ...
    return {
      ...
      thoughts: [{
        step: 1,
        thought: 'API failure - using fallback response',
        confidence: 0.1
      }],
      treeOfThought: {
        id: 'fallback',
  ...
  public async executeAction(action: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
  ...
  private async scheduleClass(params: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      // Attach to latest timetable so UI fetching /api/latest-timetable includes this row
      let latestTimetableId: string | null = null;
      try {
      ...
      let assignedTeacher = params.teacher as string | undefined;
      if (!assignedTeacher || assignedTeacher === 'To be assigned') {
        assignedTeacher = await this.findAvailableTeacherForSubject(
          params.subject,
          params.class,
          params.day || 'Monday',
          params.period
  ...
  private async cancelClass(params: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
  ...
  private async rescheduleClass(params: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      const { data, error } = await this.supabase
        .from('timetable_classes')
        .update({
  ...
  private async swapClasses(_params: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      const { eventAId, eventBId } = _params || {};
      if (!eventAId || !eventBId) return { success: false, message: 'Missing event ids for swap' };
      ...
      if (error || !events || events.length !== 2) return { success: false, message: 'Unable to load events for swap' };
      const [a, b] = events[0].id === eventAId ? [events[0], events[1]] : [events[1], events[0]];
      // Compute swapped params
      const aNew = { day: b.day, period: b.period, room: b.room } as any;
      const bNew = { day: a.day, period: a.period, room: a.room } as any;
      ...
      if (aConflict || bConflict) return { success: false, message: 'Swap would create conflicts' };
      ...
      if (updA.error) return { success: false, message: `Swap failed (A): ${updA.error.message}` };
  ...
  private async assignSubstitute(params: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      const { data, error } = await this.supabase
        .from('timetable_classes')
        .update({
  ...
  private async addConstraint(params: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      const { data, error } = await this.supabase
        ...
        .select()
        .single();
      if (error) return { success: false, message: `Error adding constraint: ${error.message}` };
  ...
  private async updateConstraint(params: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      const { id, ...rest } = params || {};
      if (!id) return { success: false, message: 'Constraint id required' };
      const { data, error } = await this.supabase
        .from('timetable_constraints')
        .update(rest)
        .eq('id', id)
        .select()
        .single();
      if (error) return { success: false, message: `Error updating constraint: ${error.message}` };
  ...
  private async removeConstraint(params: any): Promise<{ success: boolean; message: string }> {
    try {
      const { id } = params || {};
      if (!id) return { success: false, message: 'Constraint id required' };
      const { error } = await this.supabase
        ...
        .delete()
        .eq('id', id);
      if (error) return { success: false, message: `Error removing constraint: ${error.message}` };
  ...
  private async getConstraints(scope?: any): Promise<any[]> {
  ...
  // ===== Simulation (no-write) =====
  private async simulateImpact(params: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
  ...
  private async insertBatchEvent(params: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      // params: { classList: string[], day, period, subject, room, teacher? }
      const rows = (params.classList || []).map((cls: string) => ({
        students: cls,
        course: params.subject,
      ...
      for (const r of rows) {
        ...
        if (conflict) return { success: false, message: `Conflict detected for ${r.students}` };
      }
      const { data, error } = await this.supabase.from('timetable_classes').insert(rows).select();
      if (error) return { success: false, message: `Error inserting batch event: ${error.message}` };
  ...
  private async accommodateUrgentEvent(params: any): Promise<{ success: boolean; message: string; data?: any }> {
    try {
      // Find first free period across preferred days/rooms/teachers minimizing conflicts
      const preferredDays: string[] = params.preferredDays || [];
      const candidateRooms: string[] = params.rooms || [];
  ...
  public async getMentionSuggestions(query: string): Promise<TimetableEntity[]> {
    const suggestions: TimetableEntity[] = [];
...
// ===== Additional agent types and helpers =====
